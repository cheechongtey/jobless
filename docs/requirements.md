...
- ✅ Dedicated `/resume` route with a create-first empty state (start new application or pick from sidebar).
- ✅ Input job posting:
  - ✅ Paste freeform text.
...
### 1a) Form state + autosave (client-side)

- ✅ Use form state management for complex inputs (react-hook-form).
- ✅ Autosave user edits with debounced persistence (IndexedDB/Dexie) to avoid excessive writes.
  - ✅ Job form autosaves title + job fields (~300ms debounce; flush on blur/unmount).
  - ✅ Resume form autosaves resume text (~300ms debounce; flush on blur/unmount).
  - 🚧 Handle multi-tab conflicts (out of scope for MVP unless needed).
...
- 🚧 Deletion UX should require confirmation (toast/dialog) for destructive actions.
  - ✅ Application delete in main content uses a toast confirmation.
  - ❌ Sidebar delete should confirm (to prevent accidental deletes).
- ❌ If storing on server: encrypt at rest and restrict access.

### 6a) Theme / UI preferences

- ✅ Dark/light theme support with system default.
- ✅ User can toggle theme (system/light/dark).
...
- Usability:
  ...
  - ✅ Core UI polish: accessible tabs and clear active states in dark mode.
- Performance:
